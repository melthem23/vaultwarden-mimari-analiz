...
```rust
// Analiz Edilen src/main.rs ve Giriş Noktası Yapısı:
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Uygulamanın loglama sistemi başlatılıyor
    init_logging()?;

    // Güvenlik ayarları ve ortam değişkenleri (.env) yükleniyor
    let config = Config::load()?;

    // Rocket Web Framework ayağa kaldırılıyor ve rotalar belirleniyor
    rocket::ignite()
        .mount("/api", routes![login, register, post_vault])
        .launch();
